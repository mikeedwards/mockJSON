...
// This array stores query string properties
var query_string = {};

// Returns an array of query string parameters for a given URL
function parseQueryString(url) {
	var qpos = url.indexOf("?");
	if ( qpos === -1 ) {
		return;
	}
	var pairs = url.substring(qpos + 1).split("&");
	for ( var i in pairs ) {
		var p = pairs[i].split("=");
...
function getRandomData(key) {
    ...
    if ( key.substr(0, 13) === 'REGEXP_MATCH_' ) {
        var pos = key.substring(13);
	if ( pos === '' || url_matches[pos] === undefined ) {
           return key;
        }
	return url_matches[pos];
...
$.ajax = function(url, options) {
    ...
    if (options.dataType === 'json') {
		...
		url_matches = options.url.match(mock.request);
                // Store the query string parameters for this request
		parseQueryString(options.url);
